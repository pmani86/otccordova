...
class PluginMediaStreamRenderer : NSObject, RTCEAGLVideoViewDelegate {
	...
	func refresh(_ data: NSDictionary) {
		...
		var videoViewWidth = Float(0)
		if let width = data.object(forKey: "videoViewWidth") as? NSNumber {
		  videoViewWidth = width.floatValue
    }

    var videoViewHeight = Float(0)
		if let height = data.object(forKey: "videoViewHeight") as? NSNumber {
		  videoViewHeight = height.floatValue
		}
