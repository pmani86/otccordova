...
class iosrtcPlugin : CDVPlugin {
    ...
    var pluginImgRenderers: [Int : PluginImgRenderer]!
    ...
    //RTCSetMinDebugLogLevel(RTCLoggingSeverity.Verbose)
        ...
        //FIXME: Function no longer exists
	...
	func MediaStream_release(_ command: CDVInvokedUrlCommand) {
        ...
        if let source = pluginMediaStream?.rtcVideoSource {
	...
	func MediaStream_toggleCamera(_ command: CDVInvokedUrlCommand) {
		NSLog("iosrtcPlugin#toggleCamera()")

		if let streamId = command.argument(at: 0) as? String {
			let pluginMediaStream = self.pluginMediaStreams[streamId]

			if let mediaStream = pluginMediaStream {
				self.queue.async { [mediaStream] in
    ...
    func new_ImgRenderer(_ command: CDVInvokedUrlCommand) {
        NSLog("iosrtcPlugin#new_ImgRenderer()")

        let id = command.argument(at: 0) as! Int

        let pluginImgRenderer = PluginImgRenderer(
            webView: self.webView!,
            eventListener: { (data: NSDictionary) -> Void in
                let result = CDVPluginResult(status: CDVCommandStatus_OK, messageAs: data as! [AnyHashable : Any])
        ...
        self.pluginImgRenderers[id] = pluginImgRenderer

        // Run it.
        pluginImgRenderer.run()
    }

    func ImgRenderer_render(_ command: CDVInvokedUrlCommand) {
        NSLog("iosrtcPlugin#MediaStreamRenderer_render()")

        let id = command.argument(at: 0) as! Int
        let imgData = command.argument(at: 1) as! String
        let pluginImgRenderer = self.pluginImgRenderers[id]
        if pluginImgRenderer == nil {
            NSLog("iosrtcPlugin#ImgRenderer_render() | ERROR: pluginImgRenderer with id=%@ does not exist", String(id))
            return
        }
        if let imgNSData = NSData(base64Encoded: imgData, options: []) {
            if let uiImage = UIImage(data: imgNSData as Data) {
                pluginImgRenderer!.render(uiImage)
            } else {
                NSLog("iosrtcPlugin#ImgRenderer_render() | ERROR: Invalid image data for UIImage")
            }
        } else {
            NSLog("iosrtcPlugin#ImgRenderer_render() | ERROR: Invalid image data for NSData")
        }
    }

    func ImgRenderer_refresh(_ command: CDVInvokedUrlCommand) {
        let id = command.argument(at: 0) as! Int
        let data = command.argument(at: 1) as! NSDictionary
        let pluginImgRenderer = self.pluginImgRenderers[id]

        if pluginImgRenderer == nil {
            NSLog("iosrtcPlugin#ImgRenderer_refresh() | ERROR: pluginImgRenderer with id=%@ does not exist", String(id))
            return;
        }
        pluginImgRenderer!.refresh(data)
    }

    func ImgRenderer_close(_ command: CDVInvokedUrlCommand) {
        NSLog("iosrtcPlugin#ImgRenderer_close()")
        let id = command.argument(at: 0) as! Int
        let pluginImgRenderer = self.pluginImgRenderers[id]
        if pluginImgRenderer == nil {
            NSLog("iosrtcPlugin#ImgRenderer_close() | ERROR: pluginImgRenderer with id=%@ does not exist", String(id))
            return
        }
        pluginImgRenderer!.close()

        // Remove from the dictionary.
        self.pluginImgRenderers[id] = nil
    }
	...
	func getUserMedia(_ command: CDVInvokedUrlCommand) {
		...
		self.queue.async {
			for (_, value) in self.pluginMediaStreams {
					if let videoSource = value.rtcVideoSource {
							videoSource.captureSession.stopRunning()
					}
			}
			self.pluginGetUserMedia.call(
				constraints,
				callback: { (data: NSDictionary) -> Void in
					self.emit(command.callbackId,
						result: CDVPluginResult(status: CDVCommandStatus_OK, messageAs: data as! [AnyHashable: Any])
	...
	func selectAudioOutputEarpiece(_ command: CDVInvokedUrlCommand) {
		...
		do {
		    let currentRoute = try AVAudioSession.sharedInstance().currentRoute.outputs[0].portType
                    NSLog("iosrtcPlugin#getAudioOutput() route is %@", String(currentRoute))
                    var output = "earpiece"
                    switch currentRoute {
                    case "Reciever":
                        output = "earpiece"
